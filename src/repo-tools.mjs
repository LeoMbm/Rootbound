...
export function registerRepoTools(server, { authorityExecutor, continuityState = null }) {
  ...
  server.registerTool(
    "codex.repo_search",
    {
      ...
      description: "Search text in an authorized project with ripgrep through official Codex command/exec. Results are globally paginated with an opaque request-bound cursor so ChatGPT can continue without re-reading earlier matches. No Codex model turn is started.",
    ...
    async ({ query, cwd, glob, maxResults, cursor, bindingRef }) => typedToolResponse(async () => {
      const scoped = bindingRef && continuityState ? continuityState.assertCwd(bindingRef, cwd) : null;
      return searchPageAuthorized({ authorityExecutor, query, cwd: scoped?.targetCwd ?? cwd, glob, maxResults, cursor });
...
export async function searchPageAuthorized({ authorityExecutor, query, cwd, glob = null, maxResults = 100, cursor = null }) {
  const authority = await authorityExecutor.resolveAuthority({ cwd, access: "readOnly", timeoutMs: 10_000 });
  const signatureInput = { query, glob, cwd: authority.effectiveCwd };
